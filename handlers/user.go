...
func (ctx *UserHandlers) SignInUser(f fiber.Ctx) error {
	userSignInReqBody := new(UserSignInReqBody)
	if err := f.Bind().Body(userSignInReqBody); err != nil {
		return fiber.NewError(fiber.StatusBadRequest, "Invalid request body")
	}

	user := ctx.Storage.FindUserByEmail(userSignInReqBody.Email)
	if user == nil {
	...
	token, err := ctx.Utils.CreateToken(user.Id, string(f.Request().Host()))
	if err != nil {
...
func (u *UserHandlers) VerifyUserEmail(f fiber.Ctx) error {
	...
	if err != nil {
		return fiber.NewError(fiber.StatusGone, "The registration link is expired or invalidated")
	}
...
func (u *UserHandlers) SignUpUser(f fiber.Ctx) error {
	...
	if user := u.Storage.FindUnverifiedUserByEmail(userSignUpReqBody.Email); user != nil {
		return fiber.NewError(fiber.StatusBadRequest, "Please activate your account via email")
	}
